      ...
      <main className="flex-grow mx-auto max-w-7xl w-full px-6 py-12">
        ...
        <div className="mb-12 flex justify-between items-end flex-wrap gap-4">
          <div>
            <span className="text-xs font-bold tracking-widest text-gold uppercase flex items-center gap-1.5">
              <Eye className="h-4 w-4" /> {t("Navigation Log")}
            </span>
            <h1 className="font-serif text-4xl font-extrabold mt-1">{t("Recently Viewed")}</h1>
            <p className="text-sm text-muted mt-2">{t("Bespoke catalog designs you evaluated recently")}</p>
          </div>
          ...
          {viewedProducts.length > 0 && (
            ...
            >
              <Trash2 className="h-4 w-4" /> {t("Clear History")}
            </button>
          )}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 luxury-card max-w-md mx-auto">
            ...
            <h3 className="font-serif text-xl font-bold">{t("No history found")}</h3>
            <p className="text-xs text-muted mt-1.5 font-light leading-relaxed">
              {t("You haven't clicked or viewed any designs in this browser session yet.")}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              >
                ...
                <div className="flex flex-col p-4 flex-grow">
                  ...
                  <div className="flex items-center justify-between mt-auto">
                    <span className="text-sm font-bold text-gold">{product.price.toFixed(2)} {t("EGP")}</span>
